...
// @Summary search timetables
// @Router /timesheet/timetable [GET]
// @Param owner formData string true "owner"
...
// @Produce json
// @Success 200 {object} SearchTimesheetsResponse
// @Failure 500 {object} kitHttp.Error
// @tags timesheet
func (c *ctrlImpl) SearchTimesheet(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()

	owner, err := c.FormVal(r, ctx, "owner", false)
	...
	timesheet, err := c.timesheetRepo.SearchTimesheet(ctx, &pb.SearchTimesheetRequest{
		Owner:          owner,
		DateFromSearch: kitGrpc.TimeToPbTS(dateFromSearch),
		DateToSearch:   kitGrpc.TimeToPbTS(dateToSearch),
	})
	if err != nil {
		c.RespondError(w, err)
		return
	}
	c.RespondOK(w, &SearchTimesheetsResponse{Timesheets: c.toTimesheetsApi(timesheet)})
}
...
// @Router /timesheet/event [GET]
// @Param timesheetId formData string true "id of timesheet"
// @Produce json
...
func (c *ctrlImpl) SearchEvents(w http.ResponseWriter, r *http.Request) {
	...
	timesheetId, err := c.FormVal(r, ctx, "timesheetId", false)
	...
	event, err := c.timesheetRepo.SearchEvents(ctx, &pb.TimesheetIdRequest{Id: timesheetId})
	if err != nil {
		c.RespondError(w, err)
		return
	}
	c.RespondOK(w, &EventSearchResponse{Events: c.toEventsApi(event)})
}
