...
func (c *ctrlImpl) toTimesheetsApi(rq *pb.SearchTimesheetResponse) []Timesheet {
	res := []Timesheet{}
	for _, result := range rq.GetTimesheets() {
		res = append(res, c.toTimesheetApi(result))
	}
	return res
}
...
func (c *ctrlImpl) toEventsApi(rq *pb.SearchResponse) []Event {
	res := []Event{}
